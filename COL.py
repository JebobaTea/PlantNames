...
def search(search_term):
    with requests.Session():
        params = {'q': search_term, 'content': 'SCIENTIFIC_NAME', 'maxRank': 'SPECIES', 'type': 'EXACT', 'offset': 0,
                  'limit': 10}

        try:
            r = requests.get('https://api.catalogueoflife.org/dataset/3LR/nameusage/search', params=params,
...
def getSynonyms(search_term):
    ...
    with requests.Session():
        ...
        try:
            ...
            try:
                r = requests.get('https://api.catalogueoflife.org/dataset/%s/taxon/%s/synonyms' % (datasetKey, taxonID),
